...
class _ShoppingScreenState extends State<ShoppingScreen> {
          ...
          for (var crop in crops) {
            if (crop["status"] == "Not Confirmed") {
              fetchedCartItems.add({
                ...
                "cropId": crop["cropId"],
                "farmerId": crop["farmerId"],
                "amount": crop["amount"]?.toDouble() ?? 0.0,
                "quantity": crop["quantity"],
                "imageUrl": crop["imageUrl"],
                "name": crop["name"],
                "category": crop["category"],
                "status": crop["status"],
                "price": crop["price"]?.toDouble() ?? 0.0,
              });
            }
          }
        }
  ...
  void calculateTotalAmount() {
    totalAmount = cartItems.fold(0.0, (sum, item) => sum + item['amount']);
  }
  ...
  String formatDate(String? date) {
    ...
    return DateTime.tryParse(date)?.toLocal().toString().split(' ')[0] ?? "N/A";
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Status : ${item['status']}',
                        style: const TextStyle(fontWeight: FontWeight.bold)),
                    Text('Order Date : ${formatDate(item['orderDate'])}'),
                  ],
                ),
              ),
              Column(
                        ...
                        child: Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              ElevatedButton(
                                onPressed: () {
                                  widget.controller.confirmOrder(context);
                                },
